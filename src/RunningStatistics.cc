#include "Validation/PlaygroundEDProducer/interface/RunningStatistics.h"

inline RunningStatistics::RunningStatistics()
{
    // state variables
    current.nevents       = 0.;
    current.mean_x        = 0.;
...
inline void RunningStatistics::add_entry(double x, double y)
{
    // evaluate
...
inline double RunningStatistics::get_correlation() { return current.correlation; }
inline double RunningStatistics::get_slope()       { return current.slope;       }
inline double RunningStatistics::get_intercept()   { return current.intercept;   }
inline double RunningStatistics::get_mean_adc()    { return current.mean_y;      }

inline void RunningStatistics::print_statistics()
{
